...
A `KudoOperator` task allows you to specify a dependency on other operators. While dependencies in general is a complicated topic, `KudoOperator` itself is about installation dependencies i.e. your operator and all the operators it depends on (including transitive dependencies) will be installed and/or removed as one unit.
